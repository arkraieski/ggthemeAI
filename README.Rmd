...
There's also a `check_theme()` function that checks if a function is actually a ggplot2 theme. However, this function only works by actually executing the theme, **so it shouldn't be used as a security measure**. I intend to add some contrast checking to that function.
